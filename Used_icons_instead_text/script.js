...
function updateWeatherUI(data) {
    for (let i = 0; i < 5; i++) {
        ...
        document.getElementById(`date${i+1}`).innerHTML = formatDate(dayData);

        // Update the Lottie animation URL
        const iconElement = document.getElementById(`icon${i+1}`);
        iconElement.innerHTML = `<lottie-player src="${getWeatherAnimationUrl(weatherCode)}" background="transparent" speed="1" class="lottie-icon" loop autoplay mode="normal"></lottie-player>`;

        // Update temperature, humidity, and windspeed
        document.getElementById(`temperature${i+1}`).innerHTML = ` ${data.daily.temperature_2m_max[i]}°C`;
        document.getElementById(`humidity${i+1}`).innerHTML = ` ${data.daily.precipitation_sum[i]}%`;
        document.getElementById(`windspeed${i+1}`).innerHTML = ` ${data.daily.windspeed_10m_max[i]} km/h`;
    }
}
